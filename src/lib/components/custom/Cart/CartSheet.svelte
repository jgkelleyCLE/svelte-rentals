...
<Sheet.Root bind:open>
	<Sheet.Trigger>
		...
		<span
			class="absolute top-1 right-3 bg-red-500 text-white rounded-full text-xs p-1 w-5 h-5 flex items-center justify-center"
			...
			>
				{#if $cartItems.length === 0}
					<div class="flex flex-col items-center justify-center gap-2 h-[50vh]">
						...
						<button
							arai-label="View Products Page"
							href="/product"
							class="bg-safariOrange hover:bg-safariOrangeHover p-2 px-6 rounded-md text-white transition duration-300 font-bold mt-8"
							onclick={() => {
								open = false;
								goto('/product');
			...
			<Sheet.Footer>
				<div class="absolute bottom-0 right-0 w-full p-4 bg-background flex flex-col items-end">
					...
					<button
						...
						class="bg-safariOrange text-white p-2 rounded-md mt-2 px-4"
						onclick={() => {
							open = false;
							goto('/cart');
						}}>View Quote</button
