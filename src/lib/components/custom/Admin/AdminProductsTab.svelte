...
<div class="p-2 w-full">
    <Table.Root>
        ...
        <div class="flex justify-center py-8 w-full">
            <div class="text-center animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-gray-900"></div>
             <!-- <PageSpinner /> -->
        </div>
        {:else}
            ...
            <Table.Row key={item._id} class={`${item.status === "Inactive" ? "bg-red-100 dark:bg-red-950/75" : null} border-2 border-gray-300 relative`}>
                
              <Table.Cell class="hidden md:table-cell"><a href={`/product/${item._id}`}><img width={75} height={50} src={item.image} alt={item.product} /></a></Table.Cell>
