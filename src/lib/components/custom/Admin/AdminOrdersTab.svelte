			...
			{#each orders as item}
				<Table.Row key={item._id}>
					...
					<Table.Cell class="hidden md:table-cell"
						>{new Date(item.createdAt).toLocaleDateString()}</Table.Cell
					>
					<Table.Cell class="hidden md:table-cell"
						>{new Date(item.eventDate).toLocaleDateString()}</Table.Cell
					>
					<Table.Cell><OrderStatusSelect {item} /></Table.Cell>
					<Table.Cell class="hidden md:table-cell"
